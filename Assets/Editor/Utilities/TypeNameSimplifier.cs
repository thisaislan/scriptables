...
namespace Thisaislan.Scriptables.Editor.Utilities
{
    ...
    internal static class TypeNameSimplifier
    {
        ...
        /// <summary>
        /// Regular expression for cleaning Unity serialization artifacts from type names.
        /// Removes PPtr&lt;$ wrappers, closing angle brackets, and backticks from generic type notation.
        /// Compiled for better performance when used repeatedly in editor tools.
        /// </summary>
        private static readonly Regex CleanTypeRegex = new Regex(@"(?:PPtr<\$|>|`)", RegexOptions.Compiled);
        ...
        /// </summary>
        internal static string GetFormattedSerializedPropertyName(SerializedProperty serializedProperty)
        {
            ...
            return $"({CleanTypeName(serializedProperty.type)})";
        }
        ...
        {
            return string.IsNullOrEmpty(input) ? input : CleanTypeRegex.Replace(input, string.Empty);
        }
    }
}
#endif
