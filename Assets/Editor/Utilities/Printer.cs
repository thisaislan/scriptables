...
namespace Thisaislan.Scriptables.Editor.Utilities
{
    ...
    internal static class Printer
    {
        ...
        private const string SerializationErrorMessage = "Serialization Error:";
        ...
        /// <summary>Prints debug information for the specified data to the Unity console.</summary>
        /// <param name="data">The data object to debug (used to get the type name).</param>
        /// <param name="datStringValue">The formatted string representation of the data to print.</param>
        internal static void PrintData(string suffix, object data)
        {
            string typeName = TypeNameSimplifier.SimplifyTypeName(data.GetType().Name);
            PrintMessage($"{suffix} - {data.GetType().Name}<{typeName}>:\n{GetStringData(data)}");
        }

        /// <summary>
        /// Converts the object's data to a formatted string representation.
        /// Handles primitives, enums, Unity objects, and JSON serialization for other types.
        /// </summary>
        /// <param name="data">The data object to debug.</param>
        /// <returns>A string representation of the object's data</returns>
        internal static string GetStringData(object data)
        {
            if (data == null)
            {
                return NullLiteral;
            ...
            {
                UnityEngine.Object unityObj = (UnityEngine.Object)data;
                return unityObj != null ? unityObj.name : NullLiteral;
            }
            ...
            {
                return $"{SerializationErrorMessage} {e.Message}";
            }
        }
