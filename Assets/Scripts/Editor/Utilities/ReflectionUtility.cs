...
namespace Thisaislan.Scriptables.Editor.Utilities
{
    ...
    {
        internal static void CopyObjectData(object source, object destination)
        {
            ...
            foreach (FieldInfo field in fields)
            {
                ...
                {
                    Debug.LogWarning($"{Consts.FAILED_TO_COPY_FIELD} {field.Name}: {e.Message}");
                }
            }
            ...
            foreach (PropertyInfo property in properties)
            {
                if (property.CanRead && property.CanWrite)
                {
                    ...
                    {
                        Debug.LogWarning($"{Consts.FAILED_TO_COPY_PROPERTY} {property.Name}: {e.Message}");
                    }
                }
            }
        }

        internal static T CreateCopy<T>(T original) where T : Data
        {
